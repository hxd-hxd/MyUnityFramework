...
namespace BatchOperationToolsUtility.Test
{
    class _BOTUClass { }
}
...
";
                ...
                // 以下通过罗斯林（Roslyn）编译器分析
                var tree = CSharpSyntaxTree.ParseText(code);
                var tree2 = CSharpSyntaxTree.ParseText(code2);

                DebugSyntaxTree(tree2);
            }
            static void DebugSyntaxTree(SyntaxTree tree)
            {
                Debug.Log(tree);
                var LineMappings = tree.GetLineMappings();
                SyntaxNode rootNode = tree.GetRoot();
                var rootType = rootNode.GetType();
                if (rootNode is CompilationUnitSyntax cus)
                {
                    var mds = cus.Members.First();
                    var mdsType = mds.GetType();
                    if (mds is NamespaceDeclarationSyntax nds)
                    {
                        //var syntaxKind = SyntaxKind.NamespaceDeclaration;
                        var nameType = nds.Name.GetType();
                        string namefstr = nds.Name.ToFullString();
                        if (nds.Name is IdentifierNameSyntax ins)
                        {
                            string name = ins.Identifier.Text;
                        }
                    }

                    //mds.DescendantNodes().OfType<NamespaceDeclarationSyntax>();
                }
            }
